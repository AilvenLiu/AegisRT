# INVARIANTS -- Constitutional Constraints for r03-device-memory-pool
...
## 5. Roadmap-Specific Invariants (r03)

- DeviceMemoryPool MUST track every allocation in an internal std::map<void*, AllocationRecord>.
- allocate() MUST return Result<void*> error (not throw) when capacity exceeded.
- deallocate() MUST detect double-free and log error (not crash).
- DeviceMemoryPool destructor MUST log (not throw) any unreleased allocations.
- DeviceCapabilities::query() MUST NOT modify device state.
- PressureLevel MUST be derived from (used / capacity) ratio, not hardcoded thresholds.
