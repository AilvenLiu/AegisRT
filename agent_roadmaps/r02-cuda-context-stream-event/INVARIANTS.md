# INVARIANTS -- Constitutional Constraints for r02-cuda-context-stream-event
...
No phase completion, optimisation, or workaround may violate these constraints.
...
## 5. Roadmap-Specific Invariants (r02)

- CudaStream and CudaEvent MUST be move-only (deleted copy constructor and copy assignment).
- CudaStream destructor MUST call cudaStreamSynchronize before cudaStreamDestroy.
- CudaEvent destructor MUST NOT throw (noexcept).
- All CUDA handles MUST be set to nullptr/0 after move or destroy.
- CudaContext MUST be constructed before any CudaStream or CudaEvent.
- TraceCollector pointer in CudaStream MUST be non-owning (raw pointer, nullable).
