# INVARIANTS -- Constitutional Constraints for r15-response-time-analysis
...
## 5. Roadmap-Specific Invariants (r15)

- Non-preemptive blocking MUST be max WCET of all lower-priority tasks (not sum).
- RTA iteration MUST terminate: if R > deadline at any iteration, return R immediately.
- RTA MUST converge: if R does not change between iterations, it has converged.
- WCRT MUST be included in AdmissionResult for all admitted tasks.
- Blocking time MUST be included in AdmissionResult::max_blocking_time.
