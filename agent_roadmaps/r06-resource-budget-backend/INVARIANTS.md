# INVARIANTS -- Constitutional Constraints for r06-resource-budget-backend
...
## 5. Roadmap-Specific Invariants (r06)

- ResourceBudget MUST have no default constructor (all fields required at construction).
- RuntimeBackend MUST be pure virtual -- zero default implementations for execute/load/unload.
- MockBackend MUST live in tests/ directory, never in src/.
- MockBackend latency MUST be deterministic given a fixed seed.
- ModelID MUST be hashable (for use in std::unordered_map).
- ResourceBudget::is_valid() MUST return false for zero memory_limit or zero stream_limit.
