# INVARIANTS -- Constitutional Constraints for r17-scheduler-core
...
## 5. Roadmap-Specific Invariants (r17)

- Scheduler MUST reject (not execute) any model not admitted by AdmissionController.
- Deadline miss MUST be detected and logged; execution continues (not aborted).
- Decision latency MUST be <100 microseconds (measured in CI benchmark).
- All scheduling decisions MUST be logged via TraceCollector with rationale.
- Scheduler MUST NOT contain scheduling logic (delegated to SchedulingPolicy).
- Coverage MUST exceed 80% for all scheduler/ source files (critical path).
