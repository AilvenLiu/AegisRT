# INVARIANTS -- Constitutional Constraints for r10-memory-planner
...
## 5. Roadmap-Specific Invariants (r10)

- AllocationPlan MUST be computed statically (at admission time, not execution time).
- generate_plan() MUST be deterministic (same inputs -> same plan).
- peak_memory in plan MUST be >= actual peak observed during execution.
- No two overlapping tensors MAY share a MemoryRegion.
- All MemoryRegion offsets MUST be aligned to 256 bytes.
- saved_memory MUST equal (sum of all tensor sizes) - peak_memory.
