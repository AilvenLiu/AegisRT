# INVARIANTS -- Constitutional Constraints for r18-trace-enhancement
...
## 5. Roadmap-Specific Invariants (r18)

- reconstruct_decision_chain() MUST work from trace data alone (no runtime state).
- AuditTrail MUST NOT store state beyond what is in TraceCollector.
- Every scheduling decision MUST have a non-empty rationale in the trace.
- MetricsAggregator MUST compute from trace data (not from live counters).
- TraceEvent::decision_chain_id MUST link all events for a single model invocation.
