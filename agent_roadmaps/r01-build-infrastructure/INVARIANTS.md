# INVARIANTS -- Constitutional Constraints for r01-build-infrastructure
...
## 5. Roadmap-Specific Invariants (r01)

- CMake minimum version MUST be 3.25 (required for CUDA language support improvements).
- Conan MUST be the sole dependency manager; no apt/yum/brew for C++ libs.
- Cross-compilation toolchain MUST target aarch64-linux-gnu (Jetson Orin).
- CUDA_CHECK macro MUST abort with file/line info on error.
- clang-format MUST enforce ASCII-only (no UTF-8 in source).
- CI MUST fail if clang-format or clang-tidy produce warnings.
