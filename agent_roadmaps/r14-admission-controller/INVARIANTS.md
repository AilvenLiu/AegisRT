# INVARIANTS -- Constitutional Constraints for r14-admission-controller
...
## 5. Roadmap-Specific Invariants (r14)

- RMS utilisation bound MUST be n * (2^(1/n) - 1) for n tasks.
- EDF utilisation bound MUST be 1.0 (sum of C_i/T_i <= 1.0).
- AdmissionResult::reason MUST be a non-empty human-readable string.
- AdmissionResult::analysis_detail MUST include the utilisation value.
- analyze() MUST log the decision via TraceCollector.
- Rejected models MUST NOT execute under any circumstances (enforced by Scheduler in r17).
