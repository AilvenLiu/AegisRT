# INVARIANTS -- Constitutional Constraints for r16-scheduling-policies
...
## 5. Roadmap-Specific Invariants (r16)

- SchedulingPolicy MUST be pure abstract (no default implementations).
- EDFPolicy MUST select earliest absolute deadline (not relative to period).
- FIFOPolicy MUST select oldest submission_time (not oldest deadline).
- Both policies MUST return std::nullopt for empty pending queue.
- Policies MUST NOT perform admission control (that is AdmissionController's role).
- Deadline miss detection MUST be a pure function (no side effects).
