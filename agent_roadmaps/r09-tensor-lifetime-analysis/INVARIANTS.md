# INVARIANTS -- Constitutional Constraints for r09-tensor-lifetime-analysis
...
## 5. Roadmap-Specific Invariants (r09)

- TensorID MUST be globally unique across all models (not just within one model).
- Two tensors overlap iff their lifetime intervals [start, end) intersect.
- SharingOpportunity MUST only be created for non-overlapping tensor pairs.
- find_sharing_opportunities() MUST be deterministic (same input -> same output).
- LifetimeAnalyzer MUST NOT modify ExecutionContext state.
